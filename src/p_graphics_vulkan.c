...
PVulkanAppRequest *_vulkan_app_request_convert(const PGraphicalAppRequest * const graphical_app_request)
{
	...
	// extensions
	vulkan_app_request->required_extensions = p_dynarr_init(sizeof(char *), 3);
	if (!graphical_app_request->headless)
	{
		p_dynarr_add(vulkan_app_request->required_extensions, P_VPTR_CAST(char *, VK_KHR_SURFACE_EXTENSION_NAME));
#ifdef PLATINUM_DISPLAY_X11
		p_dynarr_add(vulkan_app_request->required_extensions, P_VPTR_CAST(char *, VK_KHR_XCB_SURFACE_EXTENSION_NAME));
#elif defined PLATINUM_DISPLAY_WIN32
		p_dynarr_add(vulkan_app_data->required_extensions, P_VPTR_CAST(char *, VK_KHR_WIN32_SURFACE_EXTENSION_NAME));
#endif // PLATINUM_DISPLAY
	}
#ifdef PLATINUM_DEBUG_GRAPHICS
	p_dynarr_add(vulkan_app_request->required_extensions, P_VPTR_CAST(char *, VK_EXT_DEBUG_UTILS_EXTENSION_NAME));
#endif // PLATINUM_DEBUG_GRAPHICS

	// layers
	vulkan_app_request->required_layers = p_dynarr_init(sizeof(char *), 1);
#ifdef PLATINUM_DEBUG_GRAPHICS
	p_dynarr_add(vulkan_app_request->required_layers, P_VPTR_CAST(char *, "VK_LAYER_KHRONOS_validation"));
#endif // PLATINUM_DEBUG_GRAPHICS
...
{
	p_dynarr_deinit(vulkan_app_request->required_extensions);
	p_dynarr_deinit(vulkan_app_request->required_layers);
...
PVulkanDisplayRequest *_vulkan_display_request_convert(const PGraphicalDisplayRequest * const graphic_display_request)
{
	...
	// extensions
	vulkan_display_request->required_extensions = p_dynarr_init(sizeof(char *), 3);
	if (!graphic_display_request->headless)
	{
		p_dynarr_add(vulkan_display_request->required_extensions, P_VPTR_CAST(char *, VK_KHR_SWAPCHAIN_EXTENSION_NAME));
	}
	// layers
	vulkan_display_request->required_layers = p_dynarr_init(sizeof(char *), 1);
#ifdef PLATINUM_DEBUG_GRAPHICS
	p_dynarr_add(vulkan_display_request->required_layers, P_VPTR_CAST(char *, "VK_LAYER_KHRONOS_validation"));
#endif // PLATINUM_DEBUG_GRAPHICS
...
{
	p_dynarr_deinit(vulkan_display_request->required_extensions);
	p_dynarr_deinit(vulkan_display_request->required_layers);
		...
		const VkPhysicalDevice device)
{
	...
	PDynarr *missing_features = p_dynarr_init(sizeof (const wchar_t *), 1);
	if (vulkan_display_request->required_features.robustBufferAccess && !device_features.robustBufferAccess) p_dynarr_add(missing_features, L"robustBufferAccess");
	if (vulkan_display_request->required_features.fullDrawIndexUint32 && !device_features.fullDrawIndexUint32) p_dynarr_add(missing_features, L"fullDrawIndexUint32");
	if (vulkan_display_request->required_features.imageCubeArray && !device_features.imageCubeArray) p_dynarr_add(missing_features, L"imageCubeArray");
	if (vulkan_display_request->required_features.independentBlend && !device_features.independentBlend) p_dynarr_add(missing_features, L"independentBlend");
	if (vulkan_display_request->required_features.geometryShader && !device_features.geometryShader) p_dynarr_add(missing_features, L"geometryShader");
	if (vulkan_display_request->required_features.tessellationShader && !device_features.tessellationShader) p_dynarr_add(missing_features, L"tessellationShader");
	if (vulkan_display_request->required_features.sampleRateShading && !device_features.sampleRateShading) p_dynarr_add(missing_features, L"sampleRateShading");
	if (vulkan_display_request->required_features.dualSrcBlend && !device_features.dualSrcBlend) p_dynarr_add(missing_features, L"dualSrcBlend");
	if (vulkan_display_request->required_features.logicOp && !device_features.logicOp) p_dynarr_add(missing_features, L"logicOp");
	if (vulkan_display_request->required_features.multiDrawIndirect && !device_features.multiDrawIndirect) p_dynarr_add(missing_features, L"multiDrawIndirect");
	if (vulkan_display_request->required_features.drawIndirectFirstInstance && !device_features.drawIndirectFirstInstance) p_dynarr_add(missing_features, L"drawIndirectFirstInstance");
	if (vulkan_display_request->required_features.depthClamp && !device_features.depthClamp) p_dynarr_add(missing_features, L"depthClamp");
	if (vulkan_display_request->required_features.depthBiasClamp && !device_features.depthBiasClamp) p_dynarr_add(missing_features, L"depthBiasClamp");
	if (vulkan_display_request->required_features.fillModeNonSolid && !device_features.fillModeNonSolid) p_dynarr_add(missing_features, L"fillModeNonSolid");
	if (vulkan_display_request->required_features.depthBounds && !device_features.depthBounds) p_dynarr_add(missing_features, L"depthBounds");
	if (vulkan_display_request->required_features.wideLines && !device_features.wideLines) p_dynarr_add(missing_features, L"wideLines");
	if (vulkan_display_request->required_features.largePoints && !device_features.largePoints) p_dynarr_add(missing_features, L"largePoints");
	if (vulkan_display_request->required_features.alphaToOne && !device_features.alphaToOne) p_dynarr_add(missing_features, L"alphaToOne");
	if (vulkan_display_request->required_features.multiViewport && !device_features.multiViewport) p_dynarr_add(missing_features, L"multiViewport");
	if (vulkan_display_request->required_features.samplerAnisotropy && !device_features.samplerAnisotropy) p_dynarr_add(missing_features, L"samplerAnisotropy");
	if (vulkan_display_request->required_features.textureCompressionETC2 && !device_features.textureCompressionETC2) p_dynarr_add(missing_features, L"textureCompressionETC2");
	if (vulkan_display_request->required_features.textureCompressionASTC_LDR && !device_features.textureCompressionASTC_LDR) p_dynarr_add(missing_features, L"textureCompressionASTC_LDR");
	if (vulkan_display_request->required_features.textureCompressionBC && !device_features.textureCompressionBC) p_dynarr_add(missing_features, L"textureCompressionBC");
	if (vulkan_display_request->required_features.occlusionQueryPrecise && !device_features.occlusionQueryPrecise) p_dynarr_add(missing_features, L"occlusionQueryPrecise");
	if (vulkan_display_request->required_features.pipelineStatisticsQuery && !device_features.pipelineStatisticsQuery) p_dynarr_add(missing_features, L"pipelineStatisticsQuery");
	if (vulkan_display_request->required_features.vertexPipelineStoresAndAtomics && !device_features.vertexPipelineStoresAndAtomics) p_dynarr_add(missing_features, L"vertexPipelineStoresAndAtomics");
	if (vulkan_display_request->required_features.fragmentStoresAndAtomics && !device_features.fragmentStoresAndAtomics) p_dynarr_add(missing_features, L"fragmentStoresAndAtomics");
	if (vulkan_display_request->required_features.shaderTessellationAndGeometryPointSize && !device_features.shaderTessellationAndGeometryPointSize) p_dynarr_add(missing_features, L"shaderTessellationAndGeometryPointSize");
	if (vulkan_display_request->required_features.shaderImageGatherExtended && !device_features.shaderImageGatherExtended) p_dynarr_add(missing_features, L"shaderImageGatherExtended");
	if (vulkan_display_request->required_features.shaderStorageImageExtendedFormats && !device_features.shaderStorageImageExtendedFormats) p_dynarr_add(missing_features, L"shaderStorageImageExtendedFormats");
	if (vulkan_display_request->required_features.shaderStorageImageMultisample && !device_features.shaderStorageImageMultisample) p_dynarr_add(missing_features, L"shaderStorageImageMultisample");
	if (vulkan_display_request->required_features.shaderStorageImageReadWithoutFormat && !device_features.shaderStorageImageReadWithoutFormat) p_dynarr_add(missing_features, L"shaderStorageImageReadWithoutFormat");
	if (vulkan_display_request->required_features.shaderStorageImageWriteWithoutFormat && !device_features.shaderStorageImageWriteWithoutFormat) p_dynarr_add(missing_features, L"shaderStorageImageWriteWithoutFormat");
	if (vulkan_display_request->required_features.shaderUniformBufferArrayDynamicIndexing && !device_features.shaderUniformBufferArrayDynamicIndexing) p_dynarr_add(missing_features, L"shaderUniformBufferArrayDynamicIndexing");
	if (vulkan_display_request->required_features.shaderSampledImageArrayDynamicIndexing && !device_features.shaderSampledImageArrayDynamicIndexing) p_dynarr_add(missing_features, L"shaderSampledImageArrayDynamicIndexing");
	if (vulkan_display_request->required_features.shaderStorageBufferArrayDynamicIndexing && !device_features.shaderStorageBufferArrayDynamicIndexing) p_dynarr_add(missing_features, L"shaderStorageBufferArrayDynamicIndexing");
	if (vulkan_display_request->required_features.shaderStorageImageArrayDynamicIndexing && !device_features.shaderStorageImageArrayDynamicIndexing) p_dynarr_add(missing_features, L"shaderStorageImageArrayDynamicIndexing");
	if (vulkan_display_request->required_features.shaderClipDistance && !device_features.shaderClipDistance) p_dynarr_add(missing_features, L"shaderClipDistance");
	if (vulkan_display_request->required_features.shaderCullDistance && !device_features.shaderCullDistance) p_dynarr_add(missing_features, L"shaderCullDistance");
	if (vulkan_display_request->required_features.shaderFloat64 && !device_features.shaderFloat64) p_dynarr_add(missing_features, L"shaderFloat64");
	if (vulkan_display_request->required_features.shaderInt64 && !device_features.shaderInt64) p_dynarr_add(missing_features, L"shaderInt64");
	if (vulkan_display_request->required_features.shaderInt16 && !device_features.shaderInt16) p_dynarr_add(missing_features, L"shaderInt16");
	if (vulkan_display_request->required_features.shaderResourceResidency && !device_features.shaderResourceResidency) p_dynarr_add(missing_features, L"shaderResourceResidency");
	if (vulkan_display_request->required_features.shaderResourceMinLod && !device_features.shaderResourceMinLod) p_dynarr_add(missing_features, L"shaderResourceMinLod");
	if (vulkan_display_request->required_features.sparseBinding && !device_features.sparseBinding) p_dynarr_add(missing_features, L"sparseBinding");
	if (vulkan_display_request->required_features.sparseResidencyBuffer && !device_features.sparseResidencyBuffer) p_dynarr_add(missing_features, L"sparseResidencyBuffer");
	if (vulkan_display_request->required_features.sparseResidencyImage2D && !device_features.sparseResidencyImage2D) p_dynarr_add(missing_features, L"sparseResidencyImage2D");
	if (vulkan_display_request->required_features.sparseResidencyImage3D && !device_features.sparseResidencyImage3D) p_dynarr_add(missing_features, L"sparseResidencyImage3D");
	if (vulkan_display_request->required_features.sparseResidency2Samples && !device_features.sparseResidency2Samples) p_dynarr_add(missing_features, L"sparseResidency2Samples");
	if (vulkan_display_request->required_features.sparseResidency4Samples && !device_features.sparseResidency4Samples) p_dynarr_add(missing_features, L"sparseResidency4Samples");
	if (vulkan_display_request->required_features.sparseResidency8Samples && !device_features.sparseResidency8Samples) p_dynarr_add(missing_features, L"sparseResidency8Samples");
	if (vulkan_display_request->required_features.sparseResidency16Samples && !device_features.sparseResidency16Samples) p_dynarr_add(missing_features, L"sparseResidency16Samples");
	if (vulkan_display_request->required_features.sparseResidencyAliased && !device_features.sparseResidencyAliased) p_dynarr_add(missing_features, L"sparseResidencyAliased");
	if (vulkan_display_request->required_features.variableMultisampleRate && !device_features.variableMultisampleRate) p_dynarr_add(missing_features, L"variableMultisampleRate");
	if (vulkan_display_request->required_features.inheritedQueries && !device_features.inheritedQueries) p_dynarr_add(missing_features, L"inheritedQueries");
	if (p_dynarr_count(missing_features) > 0)
	{
		p_log_message(P_LOG_ERROR, L"Platinum", L"Vulkan required features not supported");
		for (uint i = 0; i < p_dynarr_count(missing_features); i++)
			p_log_message(P_LOG_ERROR, L"Platinum", L"\t%ls", p_dynarr_get(missing_features, wchar_t *, i));
		exit(1);
	}
	p_dynarr_deinit(missing_features);
 ...
 */
static PDynarr *_vulkan_get_required_extensions(const PDynarr * const required_extensions)
{
	PDynarr *extensions = p_dynarr_init(sizeof (char *), p_dynarr_count(required_extensions));
	for (uint i = 0; i < p_dynarr_count(required_extensions); i++)
	{
		char *extension = p_dynarr_get(required_extensions, char *, i);
		if(_vulkan_extension_exists(extension))
			p_dynarr_add(extensions, P_VPTR_CAST(char *, extension));
		else
		{
 ...
 */
PDynarr *_vulkan_get_required_layers(const PDynarr * const required_layers)
{
	PDynarr *enabled_layers = p_dynarr_init(sizeof (char *), p_dynarr_count(required_layers));
	for (uint i = 0; i < p_dynarr_count(required_layers); i++)
	{
		char *layer = p_dynarr_get(required_layers, char *, i);
		if(_vulkan_layer_exists(layer))
			p_dynarr_add(enabled_layers, P_VPTR_CAST(char *,layer));
		else
		{
		...
		const PVulkanSwapchainSupport swapchain_support)
{
	...
	// Set extent
	vulkan_display_data->swapchain_extent.width = P_MIN(
			P_MAX(framebuffer_width, swapchain_support.capabilities.minImageExtent.width),
			swapchain_support.capabilities.maxImageExtent.width);

	vulkan_display_data->swapchain_extent.height = P_MIN(
			P_MAX(framebuffer_height, swapchain_support.capabilities.minImageExtent.height),
	...
	vkGetSwapchainImagesKHR(vulkan_display_data->logical_device, vulkan_display_data->swapchain, &image_count,
			swapchain_images);
	vulkan_display_data->swapchain_images = p_dynarr_init_arr(sizeof (VkImage), image_count, swapchain_images);

	// Create image views
	vulkan_display_data->swapchain_image_views = p_dynarr_init(sizeof (VkImageView), image_count);

	for (uint i = 0; i < image_count; i++)
	{
		VkImageViewCreateInfo image_view_create_info = {0};
		image_view_create_info.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
		image_view_create_info.image = p_dynarr_get(vulkan_display_data->swapchain_images, VkImage, i);
		...
		p_dynarr_add(vulkan_display_data->swapchain_image_views, &image_view);
	}
		...
		const VkSurfaceKHR surface)
{
	...
	if (format_count != 0)
	{
		...
		PDynarr *surface_formats = p_dynarr_init_arr(sizeof (VkSurfaceFormatKHR), format_count, formats);

		VkSurfaceFormatKHR format = p_dynarr_get(surface_formats, VkSurfaceFormatKHR, 0);
		for (uint i = 0; i < format_count; i++)
		{
			VkSurfaceFormatKHR current_format = p_dynarr_get(surface_formats, VkSurfaceFormatKHR, i);
		...
		*swapchain_support.format = format;
		p_dynarr_deinit(surface_formats);
	}
	...
	if (present_mode_count != 0)
	{
		...
		PDynarr *present_modes = p_dynarr_init_arr(sizeof (VkPresentModeKHR), present_mode_count, modes);

		VkPresentModeKHR present_mode = p_dynarr_get(present_modes, VkPresentModeKHR, 0);
		for (uint i = 0; i < present_mode_count; i++)
		{
			VkPresentModeKHR current_present_mode = p_dynarr_get(present_modes, VkPresentModeKHR, i);
...

{
	...
	if (p_dynarr_find(vulkan_display_data->compatible_devices, &physical_device.handle) == -1)
	{
	...
	PDynarr *queue_family_infos = p_dynarr_init(sizeof (PVulkanQueueFamilyInfo), 1);
	for (uint i = 0; i < P_VULKAN_QUEUE_TYPE_MAX; i++)
	{
		bool unique = true;
		for (uint j = 0; j < p_dynarr_count(queue_family_infos); j++)
		{
			if ((p_dynarr_get(queue_family_infos, PVulkanQueueFamilyInfo, j).exists) &&
				vulkan_display_data->queue_family_infos[i].index ==
				p_dynarr_get(queue_family_infos, PVulkanQueueFamilyInfo, j).index)
			{
				unique = false;
				break;
			}
		...
		if (unique)
			p_dynarr_add(queue_family_infos, &vulkan_display_data->queue_family_infos[i]);
	}

	const uint num_queue_families = p_dynarr_count(queue_family_infos);
	...
	for (uint i = 0; i < num_queue_families; i++)
	{
		...
		queue_create_infos[i].queueFamilyIndex = p_dynarr_get(queue_family_infos, PVulkanQueueFamilyInfo, i).index;
		queue_create_infos[i].queueCount = 1;
		queue_create_infos[i].pQueuePriorities = P_PTR_CAST(float, 1.f);
	}
	...
	PDynarr *enabled_extensions = _vulkan_get_required_extensions(vulkan_display_request->required_extensions);
	device_create_info.enabledExtensionCount = p_dynarr_count(enabled_extensions);
	device_create_info.ppEnabledExtensionNames = p_dynarr_get_arr(enabled_extensions);

	PDynarr *enabled_layers = _vulkan_get_required_layers(vulkan_display_request->required_layers);
	device_create_info.enabledLayerCount = p_dynarr_count(enabled_layers);
	device_create_info.ppEnabledLayerNames = p_dynarr_get_arr(enabled_layers);

	_vulkan_display_request_destroy(vulkan_display_request);

	// Set logical device queues
	PDynarr *device_queue_create_infos = p_dynarr_init(sizeof (VkDeviceQueueCreateInfo), 1);
	for (uint i = 0; i < p_dynarr_count(queue_family_infos); i++)
	{
		VkDeviceQueueCreateInfo queue_create_info = {0};
		queue_create_info.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
		queue_create_info.queueFamilyIndex = p_dynarr_get(queue_family_infos, PVulkanQueueFamilyInfo, i).index;
		queue_create_info.queueCount = 1;
		queue_create_info.pQueuePriorities = P_PTR_CAST(float, 1.f);
		p_dynarr_add(device_queue_create_infos, &queue_create_info);
	}
	device_create_info.queueCreateInfoCount = p_dynarr_count(device_queue_create_infos);
	device_create_info.pQueueCreateInfos = p_dynarr_get_arr(device_queue_create_infos);
	...
	p_dynarr_deinit(enabled_extensions);
	p_dynarr_deinit(enabled_layers);
	p_dynarr_deinit(device_queue_create_infos);
	p_dynarr_deinit(queue_family_infos);
		...
		const PGraphicalDisplayRequest * const graphical_display_request)
{
	...
	if (vulkan_display_data->compatible_devices != NULL)
		p_dynarr_deinit(vulkan_display_data->compatible_devices);
	{
		...
		vulkan_display_data->compatible_devices = p_dynarr_init_arr(sizeof (VkPhysicalDevice),
				vulkan_available_device_count, compatible_devices);
	}
	PDynarr *compatible_devices = vulkan_display_data->compatible_devices;

	// check device suitability, assign scores
	PDynarr *device_score = p_dynarr_init(sizeof (int), p_dynarr_count(compatible_devices));
	for (uint i = 0; i < p_dynarr_count(compatible_devices); i++) {
		int score = 0;
		VkPhysicalDevice device = p_dynarr_get(compatible_devices, VkPhysicalDevice, i);
...
end_device_score_eval:
		p_dynarr_add(device_score, P_VPTR_CAST(int, score));
	}

	// remove incompatible gpus and set default gpu
	int max_score = 0;
	int max_score_index = -1;
	for (uint i = 0; i < p_dynarr_count(compatible_devices);)
	{
		int current_device_score = p_dynarr_get(device_score, int, i);
		if (current_device_score > max_score)
		{
			...
			p_dynarr_remove_ordered(compatible_devices, i);
			p_dynarr_remove_ordered(device_score, i);
		} else {
			i++;
		}
	}
	p_dynarr_deinit(device_score);

	if (max_score_index >= 0)
	{
		PGraphicalDevice device = (PGraphicalDevice){0};
		device.handle = p_dynarr_get(compatible_devices, VkPhysicalDevice, max_score_index);
...
PGraphicalAppData p_graphics_vulkan_init(PGraphicalAppRequest *graphical_app_request)
{
	...
	vk_app_info.pApplicationName = "Platinum";
	...
	PDynarr *enabled_extensions = _vulkan_get_required_extensions(vulkan_app_request->required_extensions);
	vk_instance_create_info.enabledExtensionCount = p_dynarr_count(enabled_extensions);
	vk_instance_create_info.ppEnabledExtensionNames = p_dynarr_get_arr(enabled_extensions);

	PDynarr *enabled_layers = _vulkan_get_required_layers(vulkan_app_request->required_layers);
	vk_instance_create_info.enabledLayerCount = p_dynarr_count(enabled_layers);
	vk_instance_create_info.ppEnabledLayerNames = p_dynarr_get_arr(enabled_layers);
	...
	{
		p_log_message(P_LOG_ERROR, L"Platinum", L"Error initializing Vulkan instance.");
		exit(1);
	}
	p_dynarr_deinit(enabled_extensions);
	p_dynarr_deinit(enabled_layers);
...
void _graphics_vulkan_pipeline_init(PGraphicalDisplayData vulkan_display_data)
{
	...
	if (vulkan_display_data->shaders != NULL)
		p_dynarr_deinit(vulkan_display_data->shaders);
	vulkan_display_data->shaders = p_dynarr_init(sizeof (VkShaderModule), 2);
	...
	p_dynarr_add(vulkan_display_data->shaders, &vertShaderModule);
	p_dynarr_add(vulkan_display_data->shaders, &fragShaderModule);
	...
	PDynarr *dynamic_states = p_dynarr_init_arr(sizeof (VkDynamicState), 2,
			(VkDynamicState[]) {VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR} );

	VkPipelineDynamicStateCreateInfo dynamic_state = {0};
	dynamic_state.sType = VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO;
	dynamic_state.dynamicStateCount = p_dynarr_count(dynamic_states);
	dynamic_state.pDynamicStates = p_dynarr_get_arr(dynamic_states);
	...
	p_dynarr_deinit(dynamic_states);

	// Set up drawing framebuffers
	uint item_count = p_dynarr_count(vulkan_display_data->swapchain_images);

	if (vulkan_display_data->swapchain_framebuffers != NULL)
		p_dynarr_deinit(vulkan_display_data->swapchain_framebuffers);
	vulkan_display_data->swapchain_framebuffers = p_dynarr_init(sizeof (VkFramebuffer), item_count);

	for (uint i = 0; i < item_count; i++)
	{
		VkImageView *attachments = p_dynarr_get_arr(vulkan_display_data->swapchain_image_views);
...
void p_graphics_vulkan_display_destroy(PGraphicalDisplayData vulkan_display_data)
{
	...
	for (uint i = 0; i < p_dynarr_count(vulkan_display_data->shaders); i++)
		vkDestroyShaderModule(vulkan_display_data->logical_device,
				p_dynarr_get(vulkan_display_data->shaders, VkShaderModule, i), NULL);
	p_dynarr_deinit(vulkan_display_data->shaders);

	p_dynarr_deinit(vulkan_display_data->compatible_devices);
	p_dynarr_deinit(vulkan_display_data->swapchain_images);

	for (uint i = 0; i < p_dynarr_count(vulkan_display_data->swapchain_image_views); i++)
		vkDestroyImageView(vulkan_display_data->logical_device,
				p_dynarr_get(vulkan_display_data->swapchain_image_views, VkImageView, i), NULL);
	p_dynarr_deinit(vulkan_display_data->swapchain_image_views);
